...
- **`AppServiceProvider`** ([app/Providers/AppServiceProvider.php](app/Providers/AppServiceProvider.php)): Uses `Gate::before` with a raw DB query to grant full bypass to users whose role has `roles.papel_sistema` = `master` (global) or `manager` (da prefeitura da sessão — filtro pelo team id do Spatie), avoiding Spatie's cache issues. Desde 2026-09-04 a checagem é pela **flag**, não pelo nome do papel (ver "Módulos × Permissões").

### Modules System

Each `Tenant` has a `modules` JSON column. Active module strings map to resource visibility and Manager role permissions.

> ⚠️ **Módulos × Permissões — catálogo único (2026-09-04, [docs/Modulos_Permissoes.txt](docs/Modulos_Permissoes.txt)):** tudo que pertence a um módulo (permissões, camadas do mapa, artefatos do "Criar Artefatos", ferramentas) está declarado em **[config/modulos.php](config/modulos.php)** e lido por **[App\Support\Modulos](app/Support/Modulos.php)** (`ativo()`, `permissaoDisponivel()`, `camadaDisponivel()`, `artefatoDisponivel()`, `ferramentaDisponivel()`, `filtrarOpcoes()`, `filtrarCamadas()`, `permissoesInativas()`, `requisitosFaltantes()`). O que o catálogo NÃO conhece é tratado como núcleo (nunca esconder por engano). **Módulo desligado some para TODO MUNDO do município, Manager incluído** (o Master não entra nos municípios; o /admin vê tudo). Onde o gate atua: (1) menu — `HasTenantModule` nos Resources + `Modulos::ativo()` no `canAccess()` das Pages (⚠️ **`canAccess()` da classe sobrepõe o do trait** — página com trait E canAccess próprio precisa checar o módulo dentro do canAccess); (2) **papéis** — `RoleResource::caixas()` é a fonte única das caixas (Create/EditRole iteram nela); caixa de módulo inativo não é oferecida e as permissões já gravadas nesses módulos são **preservadas** no save (`EditRole::mutateFormDataBeforeSave`); (3) **mapa** — blade com `@if (Modulos::…)` em acordeons, checkboxes de camada, grupos/botões do Criar Artefatos, itens de Ferramentas, "Salvar como" e Filtros (elemento de módulo inativo NÃO é renderizado), `window.mapConfig.modulos`/`camadasModulo`, `/gis/{slug}/map-permissions` devolve `modulos` + `camadas_indisponiveis` e filtra `layers` pelo módulo ANTES do bypass, `MapDataController` responde **403** para camada de módulo inativo; (4) **dashboard** — widgets com `canView()` por módulo; (5) **/admin → Prefeitura → Módulos** — opções vindas do catálogo + aviso de pré-requisito (`requer`: imobiliario → base_cartografica; coleta_cadastral/pgv → imobiliario), nada é bloqueado. **Papéis de sistema pela FLAG, não pelo nome (D7):** `roles.papel_sistema` (`master` global | `manager` da prefeitura) é o que o `Gate::before`, o `map-permissions`, o `ColetaRegiaoService` e o "Delegar Manager" checam (o Gate::before agora filtra pelo município da sessão — Manager em A não é Manager em B); o Manager pode ser **renomeado**, não excluído. `roles.todos_modulos` (Toggle "Acompanha todos os módulos", qualquer papel; o Manager nasce com true) = recebe automaticamente as permissões de todo módulo ligado depois (`Tenant::sincronizarPapeisTodosModulos()` no `Tenant::updated`; `Role::managerDe($tenantId)`); ao salvar a prefeitura com módulos alterados, o `EditTenant::afterSave` avisa quais papéis NÃO acompanham e precisam de permissão manual. **Deploy:** `migrate` (flags dos papéis com backfill Master/Manager) + `PermissionsSeeder` (ver_camada_coleta/chamados/areas_reurb) + `permission:cache-reset` + **`php artisan modulos:backfill --executar`** (prefeitura com `imobiliario` ganha `base_cartografica` + `coleta_cadastral`; com panorâmicas ganha `imageamento`) + conferir `modules` de cada prefeitura no /admin (Piúma = `base_cartografica` + `mob_infra`). Teste: `php artisan test --filter ModulosTest`.

| Module key | Covers |
|---|---|
| `base_cartografica` | **Base cartográfica**: Distritos/Perímetro, Bairros, Logradouros e **Seções de Logradouro** (D8) — grupo de menu "Base Cartográfica", acordeon "Base Cartográfica" do mapa (abre por padrão); pré-requisito do imobiliário |
| `imobiliario` | **Cadastro Imobiliário**: Lotes, Quadras, Loteamentos, Zonas, Meio-fio, Áreas REURB (grupo "Módulo Imobiliário") + Consultas de Viabilidade + Customizações (grupos próprios, mesmo módulo); acordeons "Cadastro Imobiliário" e "Zoneamento Urbano" no mapa; exige `base_cartografica` |
| `coleta_cadastral` | **Coleta Cadastral**: Boletim, Regiões, Monitoramento, Produtividade, Validação, Mensagens, camada Coleta; exige `imobiliario` |
| `chamados` | **App de Chamados** (módulo próprio desde D8, 2026-09-05): Categoria/Fluxo/Chamado + camada `chamados` (acordeon Infraestrutura); sem pré-requisito |
| `imageamento` | **Imageamento**: Pontos Panorâmicos 360 + Visualizador 3D/LiDAR — grupo de menu próprio "Imageamento" e acordeon próprio no mapa (D8) |
...
| `patrimonios` | TipoPatrimônio, PatrimônioPúblico |
...
| `processos` | BpmnFluxo, ProcessoDigital, Setor, Progresso dos Processos |
| `mob_infra` | **Mobilidade Urbana** (Piúma/ES): MobTrecho (levantamento) + MobVia (fluxo/sentido, FK `mob_trechos.via_id`), MobSinalizacao + catálogo MobTipoSinalizacao, MobPontoInteresse, MobEixo, MobZona, MobFluxo, MobCamera — backlog/decisões em [docs/piuma.txt](docs/piuma.txt) |

> **D8 — Menu × Mapa × Criar Artefatos contam a mesma história (2026-09-05, [docs/Modulos_Permissoes.txt](docs/Modulos_Permissoes.txt) §9):** a ordem dos grupos da sidebar (`AppPanelProvider::navigationGroups()`), a ordem dos acordeons da janela "Camadas do Mapa" e os grupos do dropdown "Criar Artefatos" usam os **mesmos nomes, a mesma ordem e o mesmo gate de módulo** — ao criar módulo/camada nova, encaixar nos três lugares com o mesmo nome. Acordeons: Base Cartográfica (abre por padrão) · Cadastro Imobiliário · Zoneamento Urbano · Coleta de Dados · Processos Digitais · Imageamento · Mobilidade Urbana · Cadastro Social · Infraestrutura (setores fiscais, REURB, patrimônios, postes, árvores, chamados) · Gestão de Cemitérios · Cadastro Rural · Anotações. No Criar Artefatos os itens vão do maior para o menor e cada linha mostra a geometria (Polígono/Linha/Ponto) à direita; Seção de Logradouro **não** tem botão lá (nasce do próprio logradouro). Dashboard: barras por bairro + pizza de logradouros por extensão (`LogradourosPieChart`) lado a lado, donut de zonas abaixo em largura total. Backfill da chave `chamados`: `php artisan modulos:backfill --executar`. **Mapa público** ([MapaPublico](app/Filament/Cidadao/Pages/MapaPublico.php) + `mapa-cidadao-engine.js`) segue a mesma ordem de acordeons, só com o que o cidadão já via, e agora é **gateado por módulo** (`$modulos = Modulos::ativos($tenant)`, tenant explícito porque o painel cidadão não tem tenancy). **Mobilidade Urbana no público = SÓ LEITURA**: 8 camadas com os estilos portados da intranet (setas de sentido, cor por destino nos fluxos), mini-filtros e simulador visual; clique abre a ficha JS `#mob-ficha-publica` (sem Livewire) e a câmera abre o player público (`cameraPublicaAction`, só `modalContent`, sem form). O público tem o **"Colorir por" dos trechos** (select + legenda, mesma lista `mobTrechoTemas` da intranet, evento `sigweb-mob-trecho-tema`), mas **não** tem coroplético dos setores nem caneta de sentido. A ficha JS é um modal centralizado (largura 720px, fundo escurecido, fecha com Esc ou clique fora; rótulo|valor em grade). Os títulos das camadas de mobilidade no público **não** têm marcador de cor nem selo "só leitura" (pedido do usuário) e levam `margin-left` inline para não colar no check. **Mapas de fundo do público = mesmo seletor da intranet** (dropdown "Mapas Base": OSM, Esri e uma opção por ortofoto ativa da tabela `ortofotos`, `switch-basemap` no `mapa-cidadao-engine.js`; a pasta legada `/mapas/{slug}/` saiu). **Azure Maps fica fora do público de propósito**: a chave iria no HTML de qualquer visitante anônimo. **Edificação no público**: com "Ver Edificações" ligado na ficha do lote, o clique na edificação abre a ficha só leitura (`fichaEdificacaoPublicaAction` → `filament/cidadao/components/ficha-edificacao-publica.blade.php`: lote/quadra, área construída e os campos customizados via `CampoCustomizadoService::colunasExport`); o engine dá prioridade a `edificacoes` no clique porque a camada auxiliar é desenhada por cima dos lotes.
...
- **`/api/gis-data`** ([app/Http/Controllers/Api/MapDataController.php](app/Http/Controllers/Api/MapDataController.php)): Main endpoint consumed by the map. Takes `tenant_id` and `layer` query params, returns GeoJSON FeatureCollections. ⚠️ **PERF (2026-08-06): NUNCA usar o accessor `$model->geo_json` em loop** — ele roda 1 query por registro (6 mil lotes × 3 acessos = ~20 mil queries = 12 s). O padrão é converter na própria query (`selectRaw('ST_AsGeoJSON(geo, 6) AS gj')`) ou em 1 query em lote por camada (ver `buildFeatureCollection`). Rotas `gis-data`/`advanced-query` passam pelo middleware [ComprimirJsonMapa](app/Http/Middleware/ComprimirJsonMapa.php) (gzip ≥50KB: 3,7 MB → 0,47 MB). Camada de 6.610 lotes: **~0,3 s de servidor**. Evolução p/ municípios 50k+ lotes: vector tiles (MVT).
...
> **Gate de recurso via Policy (importante):** um Resource **sem Policy** fica visível para qualquer usuário (o `HasTenantModule::canAccess()` só filtra por módulo; sem Policy o Gate libera por padrão). Todo Resource novo que apareça no menu precisa de `app/Policies/{Model}Policy.php` + permissão no `PermissionsSeeder` + checkbox na `RoleResource` + whitelist no `EditRole::mutateFormDataBeforeFill`.
>
> ⚠️ **Permissão nova em Policy/canAccess = usar `User::temPermissao()` (incidente de produção 2026-08-05):** `hasPermissionTo()` (e o `can()` em algumas rotas do Spatie) **LANÇA `PermissionDoesNotExist`** quando o nome não está na tabela `permissions` — e como Policies/`canAccess` rodam na montagem do MENU, um deploy sem rodar o `PermissionsSeeder` derrubava TODAS as telas para usuários sem bypass (Master/Manager não viam porque o `Gate::before` responde antes). `temPermissao()` nega com falha fechada, sem 500. **Todo deploy que traga permissão nova DEVE rodar `php artisan db:seed --class=PermissionsSeeder` + `php artisan permission:cache-reset`** (seeder idempotente — `firstOrCreate`). Recursos gated por permissão única "gerenciar": `AreaReurbResource` (`gerenciar_areas_reurb`), `SecaoLogradouroResource` (`gerenciar_secoes_logradouro`), `ViabilidadeEmissaoResource` (view-only, `view_viabilidade_emissoes`).
...
> **Exclusão DEFINITIVA de prefeitura (2026-08-27):** a ação "Excluir prefeitura (definitivo)" do `TenantResource` (só Master; bulk delete REMOVIDO) usa o [TenantExclusaoService](app/Services/Saas/TenantExclusaoService.php): prévia de impacto (contagem por tabela via information_schema) + confirmação digitando o slug. Motor: ~97 FKs de `tenant_id` têm `cascadeOnDelete` (o Postgres apaga o grosso ao deletar a linha de `tenants` — que **nunca teve** soft delete); o service cobre o resto — varredura genérica pós-cascata (tabelas sem FK, ex. viabilidades, e as do Spatie por team — o papel Manager do tenant fica órfão sem ela), **usuários exclusivos** apagados FÍSICO (⚠️ proteção inegociável: papel global — Master/Operador — ou vínculo com outra prefeitura ⇒ só desvincula; o Master fica vinculado a várias tenants p/ acessar o /app) + tokens/notificações/pivôs, e **arquivos coletados ANTES do delete** (colunas de caminho conhecidas × schema real, genérico) + logo + mock tributário + `public/nuvem-pontos/{slug}` + prefixo `{slug}/` no R2 (best-effort). Complemento: **`php artisan tenant:limpar-orfaos`** (simulação por padrão; `--executar` aplica) caça restos de exclusões antigas: linhas com tenant_id inexistente, usuários sem vínculo e sem papel global, mocks/nuvem-pontos/prefixos R2 órfãos. Dump do banco (phpPgAdmin) antes de excluir produção é responsabilidade do operador — não há backup automático.
...
- ⚠️ **Autocura no `sincronizarAcessoAdmin` (incidente VPS 2026-08-11):** se o papel global (`Operador`/`Master`) ou uma permissão `admin_*` **não existir** na tabela (deploy sem rodar o `AdminAcessoSeeder`), o método agora **cria na hora** via `DB::table` (nunca `Role::create` — o resolver de teams do Spatie injetaria o tenant da sessão). Antes, o vínculo era pulado EM SILÊNCIO: o usuário nascia sem papel e o login no `/admin` falhava com **"credenciais inválidas"** (o Filament mascara `canAccessPanel` falso com a mesma mensagem de senha errada — armadilha de diagnóstico). O seeder continua valendo como implantação padrão (idempotente).
...
| Endpoint | Descrição |
|---|---|
| `POST /api/login` | Autenticação — retorna token + tenant (map_lat/lon/zoom + **`ortofotos`** [{id,nome,url}] ativas) + layers + **`coleta`** (boletim + região) |
...
- **Bucket de mídias — Cloudflare R2 (2026-08-24):** disk **`midia`** em `config/filesystems.php` (driver s3, `region auto`, path-style, bucket `sigweb-midia`) com credenciais do `ApiSetting` "Cloudflare R2" (`R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET`, `R2_ENDPOINT`) injetadas no boot. Pacote `league/flysystem-aws-s3-v3` (é o PROTOCOLO S3 — o endpoint aponta p/ R2, nada passa pela AWS). Estrutura interna: **prefeitura-primeiro** `{tenant_slug}/panoramicas/...` (decisão 2026-08-24: medição/offboarding por prefixo único). Bucket privado; leitura via `temporaryUrl()`. Upload em massa: rclone direto ao bucket (nunca pela VPS).
- **Panorâmicas 360 em massa (Bom Princípio/Líder, 2026-08-24):** ação **"Importar Panorâmicas 360"** no `TenantResource` (gate `admin_importar_gis`) consome o **GeoJSON de imageamento** (features POINT com `image_name`, `trajectory`, `azimuth`, `altitude`, `start_time`; levantamento real = 46.076 fotos ≈ 310GB) via [ImportadorPanoramicasService](app/Services/Gis/ImportadorPanoramicasService.php): bulk insert em chunks de 500, **idempotente por `image_name`** (reimportar o mestre atualizado = só cria os novos), `image_path` DETERMINÍSTICO `{slug}/panoramicas/{dia}/{image_name}.jpg` (dia = prefixo do trajectory, espelha as pastas do fornecedor — as fotos sobem via rclone nesse caminho e se encaixam). Colunas novas em `pontos_panoramicos`: `azimuth` (→ `northOffset` do Pannellum, bússola correta), `altitude`, `trajectory` + índice (tenant_id, titulo). `PontoPanoramico::imagem_url` (accessor, FORA de $appends — não assinar 46k URLs no payload): `image_url_simulacao` → URL assinada R2 (caminho com `/panoramicas/`) → storage local. Escala p/ 46k pontos (2026-08-24): case `pontos_panoramicos` ENXUTO no MapDataController (query direta sem hidratar Eloquent, só id/name/layer — 46k em ~200ms / 0,56MB gzip) + estilo por zoom no engine (bolinha 15–17, ícone de câmera ≥17, estilos cacheados). Viewer degrada sem foto: accessor checa `exists()` no bucket → badge "Foto ainda não enviada" (nunca erro). ⚠️ **CORS do bucket é OBRIGATÓRIO** (Pannellum carrega a imagem via WebGL/crossOrigin): R2 → bucket → Settings → CORS policy com GET/HEAD liberados — sem isso o navegador recusa a foto que o servidor serve normalmente. **Navegação estilo STREET VIEW (2026-08-24):** viewer em [panorama-viewer.blade.php](resources/views/filament/pages/panorama-viewer.blade.php) com SETAS dentro do panorama (hotspots Pannellum, css `.pano-seta`) → clique chama `$wire.panoramaDados(id)` (`HasPontoPanoramicoActions::montarDadosPanorama`) e troca a foto SEM fechar o modal. Vizinhos: anterior/próximo da MESMA trajetória (ordenação por titulo) + ponto mais próximo de CADA outra trajetória num raio de 15m (`ST_DWithin` + `ST_Azimuth` p/ bearing); setas <20° deduplicadas, máx 4; yaw da seta = bearing − azimuth. Ponto sem foto → url null → aviso âmbar (sem quebrar). URLs assinadas memoizadas (Cache 5h) = revisitar ponto usa o cache HTTP do navegador.
...
- **App RN** (`C:\laragon\www\aplicativos-sigweb\app-coletas`): form é config-driven (`/api/coleta/config`) e se adapta sozinho; `CONFIG_PADRAO` do fallback atualizado. O push antigo (campos soltos) é absorvido por shims de compat no `LoteSyncController`. ⚠️ **OOM das fotos base64 (incidente Jucurutu 2026-08-25):** o app fechava sozinho após "Ficha salva!" (intermitente, só no APK, "limpar cache resolvia") — o MapScreen recarregava TODOS os pendentes com `SELECT *` (3 fotos base64 + JSONs por ficha) a cada volta ao mapa e o Android matava o processo por memória. Correção: `getLotesPendentesLeves()` (sem fotos) no MapScreen/SyncScreen + push lendo a ficha completa UMA por vez (`getLoteById` no loop). Padrão a manter: **coluna de foto base64 nunca entra em SELECT de lista**. **Pacote de correções de campo (2026-08-26):** (a) mapa **offline-first** — fetch por bbox falhou (sem internet) → desenha a base local do SQLite (`getLotesBaseLeves`, cache com centróide + filtro de viewport em JS); antes, fechar o app offline deixava só os pendentes no mapa; (b) fotos base64 **também não entram em params de navegação** — bottom sheet usa `getLoteResumo` (ficha sem fotos) e a EditLote hidrata tudo do SQLite no mount (state `loteBase`; o save usa `loteBase`, nunca o param cru — corrige data-loss ao reabrir ficha pendente); (c) área edificação > área do lote **não bloqueia mais o salvar** — aviso + oferta de marcar Inconformidade (área construída pode superar o terreno: 2+ pavimentos); (d) edge-to-edge do SDK 54: FABs somam `insets.bottom` (a barra do Android tampava o botão Sync); (e) labels de nº do lote: o app agora lê `properties.name` da camada lotes do `MobileMapDataController` (lia `numero_lote`, que não existe) e a visibilidade por zoom virou state (ref no render não re-renderizava); teto de render 500→300 polígonos. **Monitoramento de crashes:** Sentry integrado no app (org `genesis-16`, projeto `react-native`, `@sentry/react-native` v7 + plugin Expo + metro wrapper; `setUser`/tag tenant no login; eventos offline sobem quando a conexão volta) — detalhes no CLAUDE.md do app.
...
- **Boletim configurável (R67-3, spec final 3 ESTADOS em 2026-08-04)** — `BoletimColetaPage` lista **TODOS os campos** de lote/edificação/unidade, cada um com um Select de uso: **"Não usar" / "Apenas leitura" / "Preencher em campo"** + Toggle **Obrigatório** (só no Preencher; **independente** do `obrigatorio` do sistema web — colunas `obrigatorio_coleta` e `leitura_coleta` em `campos_customizados` E `campo_dominios`). Quatro famílias de item: (1) **campos padrão** com domínio (ex.: `lote.ocupacao` — flags no CampoDominio); (2) **campos base** (fotos ×3, observação, área construída da edificação) em `tenant.data['coleta_campos_base_config']` = `{entidade: {campo: {na_coleta, leitura, obrigatorio}}}`; (3) **dados do cadastro oficial** (`ColetaConfigService::CAMPOS_LEITURA` — área/testada do lote; endereço/inscrição/código/edifício/**proprietário atual (pessoas)** + fiscais visíveis da unidade) que só oferecem Não mostrar/Apenas leitura (divergência entra por campo customizado, nunca editando o oficial) — seleção em `tenant.data['coleta_leitura']`; (4) **campos customizados** (flags na própria linha). **`GET /api/coleta/config`**: `campos_padrao`/`campos_customizados` levam `leitura` bool, `campos_base_config` leva o trio, `leitura` = mapa `{lote, unidade}` de `[{campo, label}]`, e **`campos_base` legado é DERIVADO** (exigido = visível E preenchível E obrigatório — app publicado nunca trava). No app, [boletimBase.js](../aplicativos-sigweb/app-coletas/src/services/boletimBase.js) (helpers puros testados em Node: `campoBaseConfig`, `camposLeitura`, `valorLeitura`) renderiza modo leitura em tudo (picker vira texto, foto vira "Ver foto", área vira linha) e a validação só exige campo visível E preenchível. **Obrigatoriedade é validada no app** — o push nunca rejeita coleta offline.
...
- **Onda 8 — App de Coletas: fluxo completo de campo (2026-08-04):** (a) **Proprietário divergente** (decisão do usuário): o pull envia `proprietario_nome`/`proprietario_cpf_cnpj` da unidade (canônico `pessoas.proprietario_id`, fallback JSON tributário) para o coletor COMPARAR; a divergência vai nos campos custom do kit `unidade.proprietario_divergente`/`cpf_cnpj_divergente` — **nunca sobrescreve o cadastro**; a prefeitura valida no web. (b) **Região reforçada**: `map/data?layer=lotes` filtrado por `ColetaRegiaoService::quadrasPermitidas` (só os delegados desenham; quadras/bairros integrais como referência) e **push blindado** — lote fora da região → `rejeitados[]` na resposta (`fora_da_regiao`|`sem_regiao_atribuida`); o app tira da fila e avisa. (c) **Antes→depois**: o push grava diff por campo em `coleta_imobiliaria.alteracoes` (jsonb, merge acumulativo por campanha via `ColetaImobiliaria::registrar`) — chaves `lote.ocupacao`, `lote.custom.<slug>`, `edificacao.<seq>.<campo>`, `unidade.<inscricao>.<slug>` (parse: 1º segmento = entidade, ÚLTIMO = campo — inscrição com pontos no miolo). (d) **[ValidacaoColetaPage](app/Filament/Pages/ValidacaoColetaPage.php)** (Coleta cadastral, gate `view_produtividade`) + [ColetaValidacaoService](app/Services/Coleta/ColetaValidacaoService.php): filtros campanha/período/coletor/status, alterações rotuladas (labels do município), seção "Divergências de proprietário" (oficial × informado), export **PDF + Excel** (3 abas) e ação **"Marcar campanha como validada"** (grava `tenant.data['coleta_validacao'][campanha]`; PDF sai carimbado) — marco formal antes da integração tributária. (e) **App**: bloco "Proprietário no cadastro oficial" na unidade, aviso de fichas rejeitadas no sync, tela **"Meu Progresso"** (drawer; SQLite local = 100% offline; helpers puros em `src/services/progresso.js`, testados em Node) e pill `✅ X/Y · Z%` no mapa. Contrato do app atualizado em [docs/appColetaCadastral.md](docs/appColetaCadastral.md).
...
**Importação GIS — como os relacionamentos são resolvidos:** a ação "Importar Mapa (GIS)" grava o `id` do GeoJSON em **`sequential_id`** (a PK `id` fica com a sequência global do Postgres) e resolve `quadra_id`/`bairro_id`/`lote_id`/… procurando `where tenant_id = <prefeitura> and sequential_id = <id do JSON>`. O motor vive em [ImportadorGisService](app/Services/Gis/ImportadorGisService.php) (T2.6, extraído do `TenantResource` para ser testável) com **3 modos**: **Adicionar** (insere tudo como novo — comportamento original), **Atualizar** (upsert por `tenant_id`+`sequential_id` — atualiza o registro VIVO existente e cria os demais; mesma PK ⇒ unidades/processos/coletas seguem apontando; feature sem geometria não apaga a geo existente; `dados_customizados` faz merge; é o modo pós-edição no QGIS) e **Substituir** (soft-delete em massa da camada ANTES de importar, na mesma transação — falhou, nada é apagado — e ao final **reconexão dos filhos por `sequential_id`**: FKs descobertas via `information_schema` — unidades, edificações, testadas, processos digitais, viabilidades, faces… — + `coleta_imobiliaria` polimórfica passam a apontar para o registro novo de mesmo número; o índice único parcial `(tenant_id, sequential_id) WHERE deleted_at IS NULL` das 9 camadas garante 1 registro vivo por número). Campos customizados (item 75) são importados nas **9 camadas** do seletor: property do GeoJSON com o nome do slug → `dados_customizados` (mapa `ImportadorGisService::ENTIDADE_CUSTOM`). Por isso cada município pode ter sua numeração começando em 1 sem colidir com os outros — **desde que a hierarquia seja importada de cima para baixo** (a numeração do seletor, 1. Distritos … 5. Quadras … 7. Lotes, é a ordem de dependência). Referência não encontrada deixa o vínculo **nulo** e é contada na notificação final (aviso persistente "N × Quadra"); ⚠️ **nunca** cair de volta no número do JSON como id global — isso amarrava o registro na entidade de OUTRA prefeitura (corrigido em 2026-07-30). Conferência pós-importação: `SELECT count(*) FROM lotes l JOIN quadras q ON q.id = l.quadra_id WHERE l.tenant_id <> q.tenant_id`.
...
- Ação "Ver detalhes": modal com tabela Antes/Depois dos campos alterados. **Coluna JSON (`dados_customizados`, `dados_tributarios`…) é explodida chave a chave** pelo [AuditoriaDiffService](app/Services/Auditoria/AuditoriaDiffService.php) (fonte única do modal E dos exports): numa atualização só entram as chaves que mudaram, com o rótulo do campo customizado do município (inclusive campo já desativado); criação/exclusão listam todas as chaves. Antes (2026-09-05) o modal e o PDF despejavam o JSON inteiro antes/depois
...
- Tabela **Quadras designadas** — uma linha por (atribuição × quadra) que se **sobrepõe** ao período (`data_inicio <= fim AND (data_fim IS NULL OR data_fim >= inicio)`): cadastrador, período da atribuição, lotes, coletados no período, coletados total, restantes e **% atendido** com barra.
- **"Atendidos" = coletados + inconformidades (decisão do usuário 2026-08-26):** inconformidade é visita CONCLUÍDA — conta no % e nos "restantes" (era incoerente: o por_cadastrador da API já contava, os totais não). Vale em `ProdutividadePage` (chave `atendidos` em linhas+resumo), `ProductividadeController` (`atendidos` + `percentual_geral`), blades tela/PDF e no app (`progresso.js` — pill do mapa e Meu Progresso). Os números continuam abertos (coletados · inconformidades) nos cards.
- **Fotos do PDF detalhado de Coletas Cadastradas (`pdf/produtividade-detalhado`)**: proporção preservada via `max-height`/`max-width` — a caixa fixa 100%×70px + `object-fit` deformava a foto vertical (o DomPDF IGNORA object-fit e esmaga em vez de recortar).
...
| `window.mostrarToastCoordenada(lat, lon)` | Toast fixo no topo com Lat/Lon + botão Copiar (ferramenta "Capturar Coordenadas" — Ferramentas; flag `window.capturaCoordAtiva` consome o singleclick com prioridade). O painel "ir para coordenada" é um input ÚNICO de texto — `irParaCoordenada` aceita string "lat, lon" colada (split em vírgula/espaço/;) |
| Toponímia — reposicionar | Modal de opções da toponímia → "Reposicionar no mapa" dispara `iniciar-mover-toponimia` → flag `window.movendoToponimiaId` consome o próximo singleclick (Esc cancela) → `Livewire.dispatch('moverToponimia')` persiste lat/lon+geo (`HasToponimiaActions::moverToponimia`). Hover na camada `toponimias` = mãozinha + anel laranja |
| `window.aplicarCorCamada(layerName)` | **Cores dinâmicas por camada (PoC AC item 7):** picker de cor injetado em cada linha elegível do painel (`CAMADAS_PERSONALIZAVEIS` = 9 camadas estruturais; zonas/coleta/processos/chamados/postes/árvores/meio-fios FORA — cor governada pelo dado). Wrapper clona o estilo original trocando stroke/fill (rótulos preservados; estados sociais do lote prevalecem via `PRESERVAR_COR_FEATURE`). Persistência: localStorage `sigweb_cores_camadas_{tenant}` + botão `#btn-restaurar-cores`. FUTURO: cor padrão por entidade editável no Resource |
| Camada `edificacoes` no painel | Checkbox "Edificações" abaixo de Lotes (liga TODAS; backend `case 'edificacoes'` e `layerConfigs.edificacoes` já existiam) — permissão herda de lotes via `LAYER_PERMISSION_ALIASES`; clique abre a modal `abrirOpcoesEdificacao`; a ficha do lote mantém o toggle por lote |
...
**Ortofotos por prefeitura (2026-08-27)** — tabela **`ortofotos`** (tenant_id cascade, `nome`, `url` = template XYZ `{z}/{x}/{y}`, `ordem`, `ativo`; model [Ortofoto](app/Models/Ortofoto.php) **sem** BelongsToTenant — é gerido no /admin). Cadastro no `TenantResource` (seção "Ortofotos (mapas de fundo)", Repeater→relationship reordenável). Cada ortofoto ativa vira: **(web)** basemap dinâmico `ortofoto_<id>` no seletor do `MapaFullscreen` (blade lista `$ortofotos`; engine cria os layers de `config.ortofotos`; o handler `switch-basemap` já liga o satélite por baixo p/ qualquer chave `ortofoto*`) e **(app)** opção no botão "Mapas" da bottom bar via `tenant.ortofotos` do login. ⚠️ **A config MANDA sozinha (decisão 2026-08-29):** a entrada fixa legada `ortofoto_2025` → `/mapas/{slug}/` foi REMOVIDA (aparecia p/ qualquer tenant apontando p/ pasta inexistente) — tenant sem cadastro = sem opção de ortofoto; **no deploy, cadastrar a ortofoto dos tenants que têm tiles na VPS** (conferir `ls public/mapas/`), senão a opção some para eles. A URL é livre (R2 público com egress grátis, `/mapas/...` da VPS, terceiros) — validada por regex `{z}/{x}/{y}`.

**Hospedagem das ortofotos — Cloudflare R2 + domínio próprio (operacional, 2026-08-29 → 09-01):** bucket público **`sigweb-ortofoto`** servido em **`https://tiles.sigwebmidia.com.br/{tenant_slug}/{nome-da-piramide}/{z}/{x}/{y}.{ext}`** (domínio `sigwebmidia.com.br` no registro.br delegado à Cloudflare; CORS GET/HEAD liberado; Cache Rule por `hostname eq tiles.sigwebmidia.com.br` com TTL de borda de 1 mês; egress zero). Upload sempre por **rclone** (remote `r2:`, `C:\Users\jesse\rclone\rclone.exe`; `--transfers 48 --checkers 32 --fast-list`), nunca pela VPS. Convenções aprendidas:
- **Formato padrão = WebP lossy q85** (≈15% do PNG, alfa preservado — Santa Cecília: 99.382 tiles, 8,74 GB PNG → 1,31 GB WebP). Pirâmide nova: `gdal2tiles.py --xyz --tiledriver=WEBP --webp-quality 85 --tilesize=512 -z 10-22 --processes=8 -w none mosaico.vrt saida/` (vários rasters = `gdalbuildvrt` antes). Conversão de pirâmide PNG já existente: GD (`imagecreatefrompng` → `imagewebp`, tiles de 0 bytes pulados) em workers paralelos — script de referência `converter-webp.php` usado em 2026-09-01.
- **Uma pirâmide por vigência, não por distrito** (Jucurutu 2026 veio em 3 pirâmides — sede + Boi Selado + Nova Santana — fundidas no bucket com cópia servidor-a-servidor `rclone copy --ignore-existing` para o prefixo da sede; só **1 tile em colisão** no z10, composto por alpha-blend). Pirâmides de áreas disjuntas só colidem nos zooms baixos.
- ⚠️ **Tile re-enviado no MESMO caminho fica preso no cache da borda por até 1 mês** (o CDN serviu um tile de 0 bytes já substituído no bucket — `cf-cache-status: HIT`). Por isso toda troca de formato/versão vai para um **prefixo NOVO** (`ortofoto-2025-webp`, `ortofoto-2026`) e a URL no cadastro é trocada; correção pontual = Purge por URL no painel da Cloudflare (Caching → Purge Cache → Custom Purge).
- **Tiles 512×512 = padrão do projeto (2026-09-01):** coluna `ortofotos.tile_size` (256|512; default do BANCO 256 p/ linhas legadas, default do FORMULÁRIO 512), Select "Tamanho do tile" no Repeater do `TenantResource`, `tile_size` no `$ortofotos` do `MapaFullscreen` e no payload `tenant.ortofotos` do login. **Não é preferência, é fato da pirâmide** (`gdal2tiles --tilesize`): 256 desenhado como 512 fica em escala errada; 256 continua existindo p/ fontes de terceiros (GeoServer/ArcGIS/Google usam 256). Convenção (igual em gdal2tiles, OpenLayers e Google Maps SDK): o tile 512 `(z,x,y)` cobre a **mesma área** do 256 `(z,x,y)` com o dobro de pixels ⇒ para a mesma nitidez o cliente pede **z−1** (¼ das requisições) e z22@512 ≡ z23@256. Engine: `ol.source.XYZ({tileSize:[512,512], minZoom:11, maxZoom:21})` (≡ 12/22 da grade 256; a view segue `maxZoom: 22`); app: `<UrlTile tileSize={512} maximumNativeZ={21} maximumZ={22}>` (acima do nativo o SDK amplia o tile em vez de mostrar buraco; servidor antigo sem o campo ⇒ 256). Pirâmide 256 existente vira 512 **sem perda** com `retile-512.php` (tile 512 `(z,x,y)` = mosaico 2×2 dos 256 `(z+1, 2x+dx, 2y+dy)`; z12..21@256 → z11..20@512 — Santa Cecília: 99.382 → 25.017 objetos). Comando p/ pirâmide nova (Líder): `gdalwarp -t_srs EPSG:3857 -dstalpha …` → `gdaladdo` → `gdal2tiles.py --xyz --tiledriver=WEBP --webp-quality 85 --tilesize=512 -z 10-21 -x --processes=8 -w none` (`-x` exclui tiles transparentes — 3 distritos disjuntos num só bbox gerariam milhões de tiles vazios; `-z …-21` @512 já é a resolução do z22@256 = teto da view web).
...
**Salvar enquadramento** chama `@this.call('salvarEnquadramento', lat, lon, zoom)` no Livewire, que grava em `tenant.data['map_lat/lon/zoom']` — o mesmo campo lido pelo mobile no login. Visível apenas para roles Master/Manager. O item 28 do TR de AC ("voltar ao zoom original do administrador") é atendido pelo botão existente **"Visão Geral (Zoom Extensão)"** — `window.zoomExtensao()` anima para `initialCenter/initialZoom`, que SÃO o `map_lat/lon/zoom` do tenant (decisão 2026-08-26: não criar botão dedicado, seria redundante); no app, o item se cumpre pelo comportamento padrão (o mapa sempre abre no enquadramento da tenant). ⚠️ Notas p/ o futuro: `window.map` NÃO existe (o `map` é const no closure do engine — expor helpers, nunca acessar de fora) e `irParaCoordenada` pisca um pin no destino (é ferramenta de localizar coordenada, não de navegação).
...
## Módulo Mobilidade Urbana (Piúma/ES — módulo `mob_infra`)

> **Fonte de verdade do módulo: [docs/piuma.txt](docs/piuma.txt)** (análise dos 20 GeoJSONs do levantamento, modelagem, decisões 6.x/P1–P6 e ondas 0–4 com status). Onda 0 (fundação) concluída em 2026-09-01; grupo Filament "Mobilidade Urbana".

- **6 entidades geográficas + 1 catálogo** cobrem os 19 layers do levantamento (bairros usam a camada existente do imobiliário — Piúma ativa `imobiliario` + `mob_infra`): [MobTrecho](app/Models/MobTrecho.php) (MULTILINESTRING, 612 trechos — estruturais em coluna: tipologia/pavimentação/estado/classe/dimensionamento; calçadas/estacionamento/vegetação = `dados_customizados` via kit), [MobSinalizacao](app/Models/MobSinalizacao.php) (POINT → FK `tipo_sinalizacao_id`; `descricao_original` preserva o texto cru da coleta por voz), [MobTipoSinalizacao](app/Models/MobTipoSinalizacao.php) (**catálogo** por tenant: name + tipo vertical/horizontal + cor + ícone + codigo_ctb; SEM SoftDeletes — FK restrict impede apagar tipo em uso, `ativo` aposenta), [MobPontoInteresse](app/Models/MobPontoInteresse.php) (POINT, 7 categorias), [MobEixo](app/Models/MobEixo.php) (MULTILINESTRING: ciclovia|eixo_comercial|rota_carga|rodovia — extras DER-ES em `dados_customizados`; extensões em METROS, exibir km), [MobZona](app/Models/MobZona.php) (MULTIPOLYGON: zona_od|quadrante|polo_industrial|setor_censitario + `origens`/`destinos` %), [MobFluxo](app/Models/MobFluxo.php) (linhas de desejo O/D; espessura ∝ `valores` no mapa). Entidades geográficas: BelongsToTenant + HasTenantSequentialId + SoftDeletes + LogsActivity + LogsGeometryChanges + `dados_customizados` JSONB/GIN + índice único parcial (tenant, sequential) — compatíveis com o ImportadorGisService.
- **Trecho × Via (Onda 6, 2026-09-03 — ⚠️ LER):** o **Trecho Viário** é o segmento do LEVANTAMENTO: a **ordem dos vértices** é a direção em que o coletor andou e **define calçada direita/esquerda** dos atributos — por isso o trecho **não tem sentido de tráfego** (coluna `sentido` removida) e **sua geometria nunca é invertida** (não existe `MobTrecho::inverterDirecao()`; sem "Inverter Sentido" no modal; a caneta e o simulador não o pegam). O **fluxo** vive na **Via Urbana** ([MobVia](app/Models/MobVia.php), `mob_vias`, MULTILINESTRING): `nome`, `sentido` = `mao_unica`|`mao_dupla`|null, `azimute` **CALCULADO** (ST_Azimuth 1º→último vértice), `inverterDirecao()` = ST_Reverse (permitido só aqui). **FK no trecho:** `mob_trechos.via_id` (nullOnDelete) — em Piúma 1:1, em outro município N trechos → 1 via. Permissões próprias `gerenciar_mob_vias`/`ver_camada_mob_vias`; `MobViaResource` "Vias Urbanas" + export `mob_via`. **Geração 1:1:** `php artisan mob:gerar-vias-dos-trechos --tenant=<slug> [--sentido=mao_dupla|mao_unica|nenhum]` (via com a MESMA geometria e MESMO `sequential_id` do trecho, vincula `via_id`, calcula metadados; idempotente). Importar Mapa GIS: camada `MobVia` (importar **antes** dos trechos — o trecho resolve `via_id` pelo número da via na property `via_id`); o `mob:normalizar-levantamento` gera `mob_vias.json` + `mob_trechos.json` (sem sentido, com `via_id`). **O importador recalcula sozinho** extensão/azimute (trechos, vias), extensão (eixos) e área (zonas) ao final de cada importação mob_* (`ImportadorGisService::recalcularMetadataMob`); `gis:recalcular-metadata` também cobre `mob_vias`. **Roteiro VPS com base que já tem trechos:** migrate + seeders → /admin "Vias Urbanas" em ADICIONAR (`mob_vias.json`) → "Trechos Viários" em ATUALIZAR (`mob_trechos.json`). Sentido real de Piúma (JSON do Pedro, 2026-09-04): 575 mão dupla / 37 mão única.
- **Permissões:** `gerenciar_mob_*` (única por entidade; Policies com `temPermissao` — falha fechada) + `ver_camada_mob_*` — CAIXA 19c "Mobilidade Urbana" e opções na CAIXA 20 da `RoleResource`, grupo `permissions_mobilidade` em Create/EditRole.
- **Seeder:** `MOB_SEED_TENANT=<slug> php artisan db:seed --class=MobilidadeSeeder` (idempotente; sem slug = todos os tenants com `mob_infra`) — catálogo de sinalização (39 tipos, cor por classe CTB: regulamentação vermelha, advertência amarela, indicação verde, estacionamento azul, horizontal roxa, "A Classificar" cinza) + kit de campos do trecho (18) e do eixo (5). **Deploy:** `migrate` + `PermissionsSeeder` + `permission:cache-reset` + `MobilidadeSeeder`.
- **Mapa (Onda 2):** 6 camadas self-contained no `MapDataController` (`mob_trechos` com `custom` = dados_customizados p/ tematização) + acordeon "Mobilidade Urbana" no painel (gated por `$temMobilidade`; toggles padrão `layer-toggle` + mini-filtros `.mob-sub-toggle` por tipo/categoria) + grupo no dropdown Criar Artefatos + 6 traits `HasMob*Actions` no `MapaFullscreen` (criar/editar dados/editar geometria/excluir; trecho tem **Inverter Sentido**). Engine: setas de mão única (a direção É a ordem dos vértices; tracejado = não classificado) — **desde 2026-09-03: ponta de seta branca c/ contorno escuro (contrasta com tema/satélite), alongada (`scale` 1×1,4), repetida a cada 70 px de tela ao longo da linha INTEIRA (distância acumulada — via reta longa ganha várias; estático = distribuição simétrica, nunca nas pontas), raio 8→12 com o zoom, `RegularShape` cacheado por raio|rotação** (`mobSetasSentido`/`mobSetaImagem`). **Simulador de fluxo:** checkbox "▶ Simular fluxo (animar setas)" abaixo do "Colorir por" → evento `sigweb-mob-fluxo-simular` → loop rAF ~30 fps avança `window._mobFluxoFase` (45 px/s) e chama `changed()` SÓ na camada `mob_trechos`; as setas "andam" no sentido da via (só visual, nada gravado). Sinalização círculo (V)/losango (H) na cor do catálogo, fluxos espessura ∝ `valores`, **"Colorir por"** nos trechos (colunas + campos do kit, legenda automática — evento `sigweb-mob-trecho-tema`). Permissões `ver_camada_mob_*` entram sozinhas no endpoint map-permissions (derivação dinâmica). ℹ️ 6 fluxos O/D são intrazonais (origem=destino, geometria vazia por natureza) — ficam fora do mapa, mas no banco/telas.
- **Filament (Onda 3):** grupo "Mobilidade Urbana" com 7 Resources CRUD-modal (`$tenantRelationshipName` + hasMany no Tenant): trechos (vocabulários = `MobTrecho::VOCABULARIOS`, fonte única com o mapa), sinalização (badge na cor do catálogo), catálogo (delete de tipo em uso é bloqueado — desativar), POIs, eixos (km), zonas, fluxos (badge Intrazonal). **Exports XLS/PDF/CSV/XML** nos 6 via [MobilidadeExportService](app/Services/Exports/MobilidadeExportService.php) (**parametrizado por entidade** — desvio consciente do padrão 1-service-por-entidade; Excel/CSV levam os campos do kit via `colunasExport`) + trait `TemExportacaoMobilidade` nas List pages (respeita filtros).
- **Camada "Vias Urbanas (sentido)" + caneta de sentido (Onda 4 → 6):** `layerConfigs.mob_vias` no visual PlanMob — azul `#2563eb` mão única (setas brancas no sentido do fluxo), vermelho `#dc2626` mão dupla (setas nas **duas** pontas), cinza tracejado sem classificação; motor genérico `mobSetasAoLongo(feature, resolution, cfg)` (setas a cada N px ao longo da linha inteira; `duplo` = 2ª família apontando para trás; `animado` = fase do simulador) alimenta `mobSetasSentido()` (vias) e `mobSetasDirecaoTrecho()` (trechos: tique discreto branco/cinza, estático, zoom ≥ 16 = direção do mapeamento). "▶ Simular fluxo" anima **só** `mob_vias` (mão dupla anda nos dois sentidos). Ferramentas → "Classificar Sentidos (Vias Urbanas)" — **barra discreta no rodapé, centralizada** (x-teleport pro body: `fixed` dentro de ancestral com transform quebra) com 3 canetas (mão única/dupla/inverter→única) + contador "sem sentido"; clique na VIA aplica na hora (`aplicarSentidoVia` no [HasMobViaActions](app/Filament/Pages/Traits/HasMobViaActions.php); engine consome o singleclick com `window.mobSentidoPen`). Com trecho e via ligados, o clique abre a **via** (`clickPriority`). Peneira dos 612 sentidos é operacional com a equipe da mobilidade. ⚠️ **Painéis flutuantes do mapa = posicionamento e cores em `style` inline (incidente 2026-09-03):** o painel `app` NÃO tem `viteTheme` — a página usa só o `public/css/filament/filament/app.css` pré-compilado, então só existem as classes Tailwind que o próprio Filament usa. `bottom-5`, `left-1/2`, `bg-sky-50`, `ring-sky-500`, `shadow-2xl`, `text-[11px]`… **não existem** e falham em silêncio (a barra "abria" fora da tela). Seguir o padrão da pílula de numeração/painel de azimutes: `style="position:fixed; …; z-index:9999"` + cores via `:style`. Reprodução headless (login + clique + `getBoundingClientRect`) com `playwright-core` apontando pro Chrome do sistema resolveu em minutos — vale repetir p/ bugs "não abre nada".
- **Ajustes pré-reunião de 08/09 (2026-09-04):** `mob_trechos.observacao` (text; modal do mapa, tela, export, importável, hover). **Fluxos O/D são SÓ LEITURA no mapa** (a tela do menu segue editável): sem "Fluxo O/D" no Criar Artefatos, clique = ficha (destino, volume, % do total geral, % dentro do destino) — `HasMobFluxoActions` só tem `opcoesMobFluxoAction`/`abrirOpcoesMobFluxo`. ⚠️ **Semântica dos fluxos (corrigida 2026-09-04):** o campo `fluxo` do levantamento é a **ORIGEM** (ponta compartilhada das linhas do grupo) → coluna `mob_fluxos.origem_regiao`; as zonas `origem_zona`/`destino_zona` são **derivadas** da geometria × zonas O/D (`MobFluxo::recalcularOrigensDestinos`, rodado pelo importador em fluxos e zonas e pelo `mob:fluxos-origem-destino`). **Cor por zona de DESTINO** = `MobFluxo::distribuicao()` (paleta `PALETA` por ordem de volume; a cor viaja na feature como `cor`, filtro por `destino_slug`) + **mini-checkboxes por destino** (`.mob-sub-toggle` de `mob_fluxos`, montados no `MapaFullscreen::mount`) com o % de cada destino. **Rótulo da linha = % do total GERAL** (`MapDataController` envia `percentual`/`total`; total inclui intrazonais sem geometria; valor 0 = sem rótulo). Setor censitário IBGE: cor âmbar `#b45309` (era cinza transparente). **Demografia por setor** (arquivo "Densidade Demográfica.json" da Líder = JSON do ArcGIS/Esri, Censo 2022): colunas `mob_zonas.populacao`/`densidade` (hab/**hectare** — o campo `Hab_Km2` do arquivo é hab/ha apesar do nome)/`renda`; o `mob:normalizar-levantamento` lê o arquivo (`lerDemografia`, Esri ou GeoJSON) e casa por `codigo` nos setores do `mob_zonas.json`; importador mapeia os 3 campos; **coroplético "Colorir setores IBGE por…"** (`MobZona::TEMAS_SETOR` → `window.mapConfig.mobSetorTemas`; engine evento `sigweb-mob-setor-tema`, 6 classes por quantil, paleta amarelo→marrom, legenda `#mob-setor-legenda`, valor rotulado no zoom ≥ 15). Fluxos O/D no mapa mostram **só percentuais** (sem volume absoluto) — o volume fica na tela do menu.
- **Direção dos trechos confirmada pela Líder (2026-09-03):** o `trechos.json` com `xi/yi/xf/yf` (SIRGAS 2000 / UTM 24S, EPSG:31984) são só o 1º e o último vértice de cada linha — 611/612 batem com a ordem dos vértices já importada (<2 m), geometria idêntica ao cru e ao normalizado; ids 274 e 321 têm o par copiado do vizinho (erro de tabela do fornecedor, geometria ok). Ou seja: **direção = ordem dos vértices já está certa no banco; o que falta é `sentido` (mão única/dupla)**, que o arquivo não traz — vem pela caneta ou por coluna que a Líder exporte. `azimute` do banco usa `ST_Azimuth` em geometria lon/lat (plano, ~2° de desvio em Piúma vs. geography) — mesmo padrão no `MemorialDescritivoService`; corrigir se o número for a relatório.
- **Afinamentos (2026-09-02):** modais dos traits em `3xl`; sub-itens do acordeon nascem desligados com **cascata da camada-mãe** (listener `.layer-toggle` mob_* sincroniza os `.mob-sub-toggle`); **hover** com destaque + tooltip nas 6 camadas (ramo `layer.startsWith("mob_")` na cadeia de hover do engine); **coordenada com copiar** nas 6 telas via trait [TemCoordenadaMob](app/Filament/Resources/Concerns/TemCoordenadaMob.php) (coluna copyable + campo 📋 no modal; ponto central = `ST_PointOnSurface`, formato "lat, lon" = o aceito pelo "ir para coordenada" do mapa). JSONs normalizados de Piúma preservados em `C:\laragon\www\docs\piuma-normalizado` p/ o carregamento na VPS.
- **Monitoramento em tempo real (Onda 5, 2026-09-03):** entidade [MobCamera](app/Models/MobCamera.php) (`mob_cameras`: POINT + `nome`, `tipo` = `embed`|`youtube`|`hls`|`imagem`, `url`, `url_snapshot`, `provedor`, `azimute_visada`, `ativo`; padrão mob completo — tenant/sequential/soft delete/auditoria/geo log, Policy `gerenciar_mob_cameras` + camada `ver_camada_mob_cameras`, CAIXA 19c/20, `MobCameraResource` "Monitoramento em Tempo Real" com export via `MobilidadeExportService` spec `mob_camera`). **Mapa:** checkbox "Monitoramento em tempo Real" logo abaixo do "Simular fluxo" (camada `mob_cameras`, `case` no `MapDataController` só com metadados — o vídeo NUNCA é carregado em massa), ícone SVG de câmera (vermelho ativa / cinza inativa) + cone de visada pelo azimute, hover "Ao vivo — clique para assistir", criação por "Criar Artefatos → Câmera de Monitoramento". Clique → `HasMobCameraActions::opcoesMobCameraAction` = modal 4xl com o PLAYER em cima (`modalContent` → [mob-camera-player.blade.php](resources/views/filament/pages/partials/mob-camera-player.blade.php): iframe p/ embed/YouTube (`MobCamera::urlPlayer()` converte watch/youtu.be/live em /embed autoplay mudo), `<video>` + hls.js (CDN jsdelivr) p/ `.m3u8`, `<img>` recarregada a cada 5 s p/ snapshot) e os dados numa `Section` recolhida + Reposicionar/Excluir. Fechar o modal destrói o iframe = stream para. ⚠️ **"Vídeo piscando" (2026-09-03):** player de terceiro (FullCam) dentro de página **HTTP** (Laragon local) entra em loop de recarga — o app deles usa `crypto.randomUUID`, que só existe em contexto seguro, e o iframe HTTPS herda a insegurança do ancestral (medido: 25 recargas do documento em 12 s, 0 requests Livewire, iframe nosso intacto). O blade só monta o iframe quando `window.isSecureContext`; senão mostra aviso + "Abrir em nova aba" + "tentar carregar mesmo assim". O box do player tem `wire:ignore` (o `->live()` do select do tipo não recria o iframe). Em produção (HTTPS) o vídeo abre direto — provado headless com iframe sob página HTTPS (1920×1080 playing). **Demo real:** Piúma publica 7 câmeras municipais (IZ1 Telecom, player FullCam `cloud.fullcam.me/#/cembed/<token>`, embed público sem X-Frame-Options/CSP) — `MobCamerasPiumaSeeder` (idempotente por tenant+nome; resolve o tenant por `MOB_SEED_TENANT=<slug>` → slug local `prefeitura-de-piuma` → único tenant com "piuma" no slug — o da VPS é `prefeitura-municipal-de-piuma`; ambíguo/ausente = erro listando os slugs, nada gravado) com posições aproximadas dos POIs (prefeitura ajusta com Reposicionar). Câmeras RTSP próprias (Intelbras/Hikvision) exigem servidor de mídia (MediaMTX → HLS/WebRTC) numa caixa na rede da prefeitura publicando p/ a VPS — o tipo `hls` já está pronto p/ isso. **Deploy:** `migrate` + `PermissionsSeeder` + `permission:cache-reset` + `MobCamerasPiumaSeeder` (só Piúma). ⚠️ Lição do dia: `bash -c` heredoc no Git Bash/Windows quebra com emoji (4 bytes) no conteúdo — arquivos com emoji vão pelo Write.
- **Importação (Onda 1):** `php artisan mob:normalizar-levantamento <pasta-crua> --saida=<pasta>` ([MobNormalizarLevantamento](app/Console/Commands/MobNormalizarLevantamento.php)) transforma os 20 JSONs crus do levantamento nos 6 GeoJSONs das camadas mob + bairros + `relatorio.txt` — de/para por palavra-chave da sinalização (99,5% classificada em Piúma), recorte das rodovias DER-ES pelo bbox municipal (29 de 896), dedup de fluxos por geometria idêntica, typos corrigidos (esuqerdo→esquerdo, Arbustro→Arbusto) e valores casados com as opções do kit. A importação usa o **ImportadorGisService** normal (camadas `Mob*` no seletor do admin): ramo próprio `mapearCamposMob()` (as regras legadas assumem colunas do imobiliário) + `resolverTipoSinalizacao()` (nome do JSON → id do catálogo do tenant, case-insensitive, fallback "A Classificar"). ⚠️ mob_fluxos NÃO tem extensao_geo (fora do RecalcularGeoMetadata). Tenant local de teste: `prefeitura-de-piuma` (carga completa validada em 2026-09-01).
