...
| `mob_infra` | **Mobilidade Urbana** (Piúma/ES): MobTrecho, MobSinalizacao + catálogo MobTipoSinalizacao, MobPontoInteresse, MobEixo, MobZona, MobFluxo — backlog/decisões em [docs/piuma.txt](docs/piuma.txt) |
...
- **Sentido/direção do trecho:** direção = **ordem dos vértices** da linha (criação usa a direção do desenho); `sentido` = `mao_unica`|`mao_dupla`|null (não classificado); **`azimute` é CALCULADO** (ST_Azimuth 1º→último vértice — `MobTrecho::atualizarMetadataGeo()`, também no `gis:recalcular-metadata`), nunca digitado; `inverterDirecao()` = ST_Reverse + recálculo. Backfill dos 612 na Onda 4 (modo de classificação em massa no mapa).
