...
<head>
    <meta charset="UTF-8">
    <title>Boletim de Cadastro Imobiliário (BIC)</title>
    <style>
        body {
    ...
    <table>
        <tr>
            <td width="33%">
                ...
                <span
                    class="value">{{ $dadosJson['inscricao_imobiliaria'] ?? $imovel->inscricao_imobiliaria ?? 'Não Informada' }}</span>
            </td>
            <td width="33%">
                <span class="label">Código Tributário</span>
                <span
                    class="value">{{ $dadosJson['codigo_imovel_tributario'] ?? $imovel->codigo_imovel_tributario ?? 'Não Informado' }}</span>
            </td>
            <td width="34%">
    ...
    <table>
        <tr>
            <td width="70%">
                ...
                <span
                    class="value">{{ $dadosJson['proprietario_name'] ?? $imovel->proprietario->name ?? 'Proprietário Não Informado' }}</span>
            </td>
            <td width="30%">
        ...
        <tr>
            <td colspan="2">
                ...
                @endphp
                <span class="value">{{ $endCorre }}, {{ $numCorre }} - {{ $bairroCorre }} -
                    {{ $munCorre }}/{{ $ufCorre }} (CEP: {{ $cepCorre }})</span>
            </td>
        </tr>
    </table>
    ...
    <table>
        <tr>
            <td width="80%">
                ...
                <span
                    class="value">{{ trim(($dadosJson['tipo_logradouro'] ?? '') . ' ' . ($dadosJson['logradouro'] ?? 'Logradouro Não Informado')) }}</span>
            </td>
            <td width="20%">
        ...
        <tr>
            <td colspan="2">
                ...
                <span class="value">
                    Quadra: {{ $imovel->lote->quadra->name ?? 'Não Vinculada' }} |
                    Lote (Terreno): {{ $imovel->lote->numero_lote ?? 'Não Vinculado' }}
                </span>
            </td>
        </tr>
    </table>
    ...
    <table>
        <tr>
            ...
            <td width="25%">
                ...
                <span
                    class="value">{{ number_format((float) ($dadosJson['area_total_edificacao'] ?? 0), 2, ',', '.') }}</span>
            </td>
            <td width="25%">
        ...
        <tr>
            <td colspan="4">
                ...
                <span class="value">{{ $dadosJson['descricao_classificacao'] ?? 'Sem Classificação' }} (Cód:
                    {{ $dadosJson['codigo_classificacao'] ?? '-' }})</span>
            </td>
        </tr>
    </table>
    ...
    <table>
        <tr>
            <td width="50%">
                ...
                <span class="value">R$
                    {{ number_format((float) ($dadosJson['valor_venal_lote'] ?? 0), 2, ',', '.') }}</span>
                <span style="font-size: 8px; color: #666;">(Ref: R$
                    {{ number_format((float) ($dadosJson['valor_metro_terreno'] ?? 0), 2, ',', '.') }}/m²)</span>
            </td>
            <td width="50%">
                <span class="label">Valor Venal da Edificação</span>
                <span class="value">R$
                    {{ number_format((float) ($dadosJson['valor_venal_edificacao'] ?? 0), 2, ',', '.') }}</span>
                <span style="font-size: 8px; color: #666;">(Ref: R$
                    {{ number_format((float) ($dadosJson['valor_metro_edificacao'] ?? 0), 2, ',', '.') }}/m²)</span>
            </td>
        </tr>
        <tr>
            <td colspan="2" class="highlight" style="text-align: center;">
                ...
                @endphp
                <span class="value" style="font-size: 16px; color: #1e3a8a;">R$
    ...
    <table>
        <tr>
            <td width="33%">
                ...
                <span class="value">R$
                    {{ number_format((float) ($dadosJson['valor_imposto_territorial'] ?? 0), 2, ',', '.') }}</span>
            </td>
            <td width="33%">
                <span class="label">Imposto Predial Lançado</span>
                <span class="value">R$
                    {{ number_format((float) ($dadosJson['valor_imposto_predial'] ?? 0), 2, ',', '.') }}</span>
            </td>
            <td width="34%" class="highlight">
                <span class="label">TRIBUTAÇÃO TOTAL (IPTU)</span>
                <span class="value" style="color: #b91c1c;">R$
                    {{ number_format((float) ($dadosJson['valor_total_imposto'] ?? 0), 2, ',', '.') }}</span>
            </td>
        </tr>
        @if(!empty($dadosJson['descricao_isento']) && $dadosJson['descricao_isento'] !== 'Não Isento')
            <tr>
                <td colspan="3" class="alert-red">
                    ...
                    <span class="value" style="color: #b91c1c;">{{ $dadosJson['descricao_isento'] }} (Cód:
                        {{ $dadosJson['codigo_isento'] ?? '-' }})</span>
                </td>
            </tr>
        @endif
    </table>
