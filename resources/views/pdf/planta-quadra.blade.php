...
<body>
    ...
    <div class="doc-header">
        <div class="right">
            ...
            @isset($sirgasUtmZone)
                @if ($sirgasUtmZone)
                    SIRGAS 2000 UTM ZONA {{ $sirgasUtmZone }}
                @endif
            @endisset
        </div>
            ...
            <div class="croqui-box">
                ...
                @else
                    <div style="display: table; width: 100%; height: 320px;">
                        <div class="croqui-placeholder">Croqui não disponível — gere a partir do mapa para incluir a
                            imagem.</div>
                    </div>
                @endif
            </div>
        ...
        <div class="col col-info">
            ...
            <div class="summary" style="margin-top: 4px;">
                ...
                <div class="cell" style="width: 33.33%; background: #f0fdf4; border-color: #bbf7d0;">
                    ...
                    <div class="value" style="color: #14532d;">{{ number_format($areaTotalLotes, 2, ',', '.') }} m²
                    </div>
                </div>
                <div class="cell" style="width: 33.33%; background: #fef2f2; border-color: #fecaca;">
                    ...
                    <div class="value" style="color: #7f1d1d;">{{ number_format($areaTotalConstr, 2, ',', '.') }} m²
                    </div>
                </div>
            </div>
        </div>
    ...
    @foreach ($lotes as $l)
        ...
        <div class="lote-card">
            <div class="lote-title">
                Lote {{ $l->numero_lote ?? '—' }}
                @if ($l->zona)
                    <span style="font-weight: normal; color: #6b7280; font-size: 9px;">· Zona
                        {{ $l->zona->sigla }}</span>
                @endif
            </div>

            @if ($l->unidadesImobiliarias->isNotEmpty())
                <div style="font-size: 9px; font-weight: bold; color: #4b5563; margin: 3px 0 2px 0;">Unidades
                    Imobiliárias</div>
    ...
    <div class="doc-footer">
        {{ $tenant?->name ?? 'Município' }} · Planta de Quadra {{ $quadra->name }} · Gerado em {{ $dataHora }} -
        Sistema SIGWEB
    </div>
