...
class RuralHidrografia extends Model {
    use HasFactory, SoftDeletes, BelongsToTenant, HasTenantSequentialId;
    protected $table = 'rural_hidrografias';
    protected $fillable = ['tenant_id', 'sequential_id', 'code', 'rural_localidade_id', 'nome', 'tipo', 'geo'];
    ...
    public function getGeoJsonAttribute() {
        ...
        $result = DB::table('rural_hidrografias')->select(DB::raw('ST_AsGeoJSON(geo) as geo_json'))->where('id', $this->attributes['id'])->first();
        return $result ? json_decode($result->geo_json) : null;
    }

    public function setGeoAttribute($value) {
        // GEOMETRY genérico: não forçamos ST_Multi aqui, apenas o GeomFromGeoJSON
