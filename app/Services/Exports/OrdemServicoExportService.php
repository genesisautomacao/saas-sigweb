...
class OrdemServicoExportService
{
    ...
    private function linha($os): array
    {
        $artefato = $os->asset_type ? class_basename($os->asset_type).' #'.($os->asset->sequential_id ?? '?') : '-';
        ...
        return [
            ...
            'Artefato' => $artefato,
            'Prioridade' => ucfirst($os->prioridade),
            'Equipe' => $os->equipe->pluck('name')->implode(', ') ?: 'Não definida',
            'Abertura' => $os->created_at?->format('d/m/Y'),
            'Conclusao' => $os->data_fim ? $os->data_fim->format('d/m/Y') : '-',
        ];
    }

    public function exportToCsv(Collection $ordens)
    {
        ...
        $data = $ordens->map(fn ($os) => $this->linha($os));
    ...
    public function exportToXml(Collection $ordens)
    {
        $fileName = 'ordens-servico-'.now()->format('Y-m-d-His').'.xml';
        $xml = new \SimpleXMLElement('<?xml version="1.0" encoding="UTF-8"?><ordens_servico/>');

        foreach ($ordens as $os) {
            $item = $xml->addChild('ordem_servico');
            foreach ($this->linha($os) as $k => $v) {
