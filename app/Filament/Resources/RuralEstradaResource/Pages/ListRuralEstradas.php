        ...
        return [
            Actions\ActionGroup::make([
                ...
                Actions\Action::make('export_csv')
                    ...
                    ->action(fn ($livewire, \App\Services\Exports\RuralEstradaExportService $s) => $s->exportToCsv($livewire->getFilteredTableQuery()->get())),

                Actions\Action::make('export_xml')
                    ->label('Exportar XML')
                    ->icon('heroicon-o-code-bracket')
                    ->action(fn ($livewire, \App\Services\Exports\RuralEstradaExportService $s) => $s->exportToXml($livewire->getFilteredTableQuery()->get())),
            ])
            ->label('Exportar')
            ...
            Actions\Action::make('nova_estrada')
                ...
                ->action(function (array $data) {
                    if ($data['metodo'] === 'mapa') {
                        ...
                        $mapUrl = url('/app/' . $tenant->slug . '/mapa-interativo?layer=rural-estradas&action=create');
