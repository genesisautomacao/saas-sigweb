   ...
   // Quando vai ABRIR A TELA (Preencher os dados)
    protected function mutateFormDataBeforeFill(array $data): array
    {
        if ($this->record->geo_json) {
            // A MÁGICA DA CORREÇÃO: Garante que o dado seja tratado 100% como Array Associativo
            ...
            if (isset($geo['type']) && $geo['type'] === 'MultiPolygon' && count($geo['coordinates']) === 1 && count($geo['coordinates'][0]) === 1) {
                ...
                // Cola na tela como lista separada por vírgula e quebra de linha
                $data['geo_json_input'] = implode(",\n", $linhas);
            } else {
                // Se for muito complexo (vários anéis/buracos), exibe o JSON formatado
